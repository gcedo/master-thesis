  ...
  draw: function(id, d, options){
    ...
    d.forEach(function(y, x){
      g.selectAll(".nodes")
        ...
        return cfg.w/2*(1-(Math.max(j.value, 0)/cfg.maxValues[i])*cfg.factor*Math.sin(i*cfg.radians/total));
        })
        .attr("cy", function(j, i){
          return cfg.h/2*(1-(Math.max(j.value, 0)/cfg.maxValues[i])*cfg.factor*Math.cos(i*cfg.radians/total));
        })
